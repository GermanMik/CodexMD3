...
def main() -> int:
    ...
    rc_results = [
        ...
        {"id": "RC8", "status": "PASS" if check_map["expected_version_alignment"]["status"] == "PASS" else "FAIL", "reason": "Plugin, MCP manifest, and MCP contracts metadata are aligned with the expected release version." if check_map["expected_version_alignment"]["status"] == "PASS" else check_map["expected_version_alignment"]["reason"]},
